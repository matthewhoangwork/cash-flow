...
create table public.transactions (
  ...
  -- Null while `planned` is true and no due date has been set yet.
  date timestamptz,
...
-- For projects created before `date` became optional (planned transactions
-- can now be created without a due date), run this once:
alter table public.transactions alter column date drop not null;

-- The standalone "monthly expenses" checklist has been folded into planned
-- transactions above; drop its table if it exists from an older install:
drop table if exists public.planned_expenses;
